...
#### Import Data (Age-depth models) ####
load("ageDepthModelsScenariosAandB.Rdata")
...

{
  ...
  simulateTraitEvo=function(t,mode,...){
    ...
    if(mode=="Stasis"){
      ...
      print("unknown mode of evolution",quote=TRUE)
      stop()
    }
  }
  ...
  getSampleAge=function(sampleLocations,ADMHeight,ADMTime){
    ...
    transVal=pointtransform(points= sampleLocations,
                            xdep=adjustADMHeight,
                            ydep=adjustADMTime,
                            direction="height to time", 
                            depositionmodel = "age model")$time
    return(transVal)
  }
  ...
  getTraitValInStrat=function(distBetweenSamples,ADMHeight,ADMTime,mode,...){
    ...
    sampleLocations=seq(from=distBetweenSamples, # determine sampling locations
                        to=max(ADMHeight),
                        by=distBetweenSamples)
    sampleTimes=getSampleAge(sampleLocations=sampleLocations, # determine ages of sampling locations
                             ADMHeight=ADMHeight,
                             ADMTime = ADMTime)
    traitInfo=simulateTraitEvo(t=sampleTimes, # simulate trait values at the age that corresponds to the sampling locations
                               mode=mode, ...)
  ...
  testModesOfEvolStrat=function(distBetweenSamples,specimensPerSample,interPopVar,noOfTests,ADMHeight,ADMTime,inputMode,...){
    ...
    for(i in 1:noOfTests){
      traitValInStrat=getTraitValInStrat(distBetweenSamples,ADMHeight,ADMTime,mode=inputMode,...)
      ## define paleots object, see https://cran.r-project.org/web/packages/paleoTS/vignettes/paleoTS_basics.html
      fossilTimeSeries = as.paleoTS(mm = traitValInStrat$traitValue, # mean value is the simulated trait values
                                    ...
                                    tt = traitValInStrat$height) # strat position
      # fit models to paleots object
      w.grw <- fitSimple(fossilTimeSeries,model = "GRW") #General Random walk (Brownian drift)
      w.urw <- fitSimple(fossilTimeSeries,model = "URW") #Unbiased Random Walk (Brownian motion)
      w.stat <- fitSimple (fossilTimeSeries,model = "Stasis") # Stasis 
      w.ou <- fitSimple(fossilTimeSeries, model = "OU") # Ornstein-Uhlenbeck model
      
      # This compares the models, the most likely model has the highest akaike.wt and the smallest AICc values.
      compared=compareModels(w.grw,w.urw,w.stat,w.ou,silent=TRUE,sort=FALSE)
      
      # store test results and input data
      testRes=list(testRes=compared, # results of model comparison
                   inputData=traitValInStrat, # raw data used as input
                   paleots=fossilTimeSeries) # paleots object
  ...
  testModesOfEvolTime=function(maxTime,noOfSamplingLocations,specimensPerSample,interPopVar,noOfTests,inputMode,...){
    ...
    for(i in 1:noOfTests){
      ...
      traitValInTime=simulateTraitEvo(t=seq(0,maxTime,length.out=noOfSamplingLocations), # simulate trait values at the age that corresponds to the sampling locations
                                      mode=inputMode, ...)
      # define paleoTS object
      fossilTimeSeries = as.paleoTS(mm = traitValInTime$traitValue,
                                    vv = rep(interPopVar,length.out=length(traitValInTime$traitValue)),
                                    nn = rep(specimensPerSample,length.out=length(traitValInTime$traitValue)),
...
#### Define models of evolution that will be tested for ####
...
EvoModes=list()
EvoModes[[1]]=list(name="stasis",
                  mode="Stasis",
                  params=c(0,1)) # stasis with mean 0, variance 1
EvoModes[[2]]=list(name="Brownian motion",
                  mode="BD",
                  params=c(0,1)) # B<M with mean zero and variance 1
EvoModes[[3]]=list(name="weak Brownian drift",
               mode="BD",
               params=c(5,1)) # BD with drift 5 and variance 1
EvoModes[[4]]=list(name="strong Brownian drift",
               mode="BD",
               params=c(10,1)) # BD with drift 10 and variance 1
...
noOfTests=400 #no of tests performed per basin position
set.seed(123) # fix seed for repeatability & debugging
...
maxTimes=list(A=2,
              B=2.58) # duration of scenarios A & B respectively in Ma
...
for (scenario in scenarioNames){
  ...
  for (dist in examinedBasinPositions){ ## for all examined transsects
    ...
    for (j in 1:length(EvoModes)){
      ...
      distFromShoreRes[[EvoModes[[j]]$name]]=testModesOfEvolStrat(distBetweenSamples = distanceBetweenSamples,
...
for (scenario in scenarioNames){
  ...
  for (noSamp in noOfSamplingLoc){
    ...
    for (j in 1:length(EvoModes)){
      ...
      noOfSamplingLocRes[[EvoModes[[j]]$name]]=testModesOfEvolTime(maxTime = maxTimes[[scenario]],
...
save(testResultsStrat,
     ...
     simulatedEvoModes,
     testedEvoModes,
     file = "resultsTestModesOfEvolution.Rdata")
